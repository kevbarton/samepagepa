...
<div class="mt-5 setup-status-wrapper">
    <a href="{{route('appusergetstarted.profile')}}" class="setup-status-block d-flex align-items-center active @if(auth()->user()->created_at < auth()->user()->updated_at) checked  @endif">
                ...
                <a href="{{route('appusergetstarted.familymembers')}}" class="setup-status-block d-flex align-items-center @if(auth()->user()->familymembers()->count()) checked @endif">
