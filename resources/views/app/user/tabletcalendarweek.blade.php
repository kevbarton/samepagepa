                ...
                <div class="row justify-content-between">
                    ...
                    <ul>
                        <li @if($weekdays[0]->active) class="active" @endif>
                            <a href="{{route('app.user.calendartabletweek',['dt'=>date('Y-m-d',$weekdays[0]->timestamp)])}}" class="text-decoration-none d-flex flex-column text-center">
                        ...
                        <a href="{{route('app.user.calendartabletweek',['dt'=>date('Y-m-d',$weekdays[1]->timestamp)])}}" class="text-decoration-none d-flex flex-column text-center">
                        ...
                        <a href="{{route('app.user.calendartabletweek',['dt'=>date('Y-m-d',$weekdays[2]->timestamp)])}}" class="text-decoration-none d-flex flex-column text-center">
                        ...
                        <a href="{{route('app.user.calendartabletweek',['dt'=>date('Y-m-d',$weekdays[3]->timestamp)])}}" class="text-decoration-none d-flex flex-column text-center">
                            <span class="day">T</span>
                        ...
                        <a href="{{route('app.user.calendartabletweek',['dt'=>date('Y-m-d',$weekdays[4]->timestamp)])}}" class="text-decoration-none d-flex flex-column text-center">
                        ...
                        <a href="{{route('app.user.calendartabletweek',['dt'=>date('Y-m-d',$weekdays[5]->timestamp)])}}" class="text-decoration-none d-flex flex-column text-center">
                            <span class="day">S</span>
                            <span class="date">{{str_pad($weekdays[5]->day,2,'0',STR_PAD_LEFT)}}</span>
                            </a>
                        </li>
                        <li @if($weekdays[6]->active) class="active" @endif>
                        <a href="{{route('app.user.calendartabletweek',['dt'=>date('Y-m-d',$weekdays[6]->timestamp)])}}" class="text-decoration-none d-flex flex-column text-center">
                            <span class="day">S</span>
                ...
                <div class="tablet-content-inner">
                    <div class="content-top-bar d-flex justify-content-between align-items-center">
                        ...
                        <div class="tabs-wrapper">
                            <ul class="tabs-block">
                                <li class="tabs-item "><a class="text-decoration-none" href="{{route('app.user.calendartablet',['dt'=>date('Y-m-d',strtotime($now))])}}">Day</a></li>
                                <li class="tabs-item active"><a class="text-decoration-none" href="{{route('app.user.calendartabletweek',['dt'=>date('Y-m-d',strtotime($now))])}}">Week</a></li>
                    ...
                    <div class="content-bottom-bar"  id="calendar-timeline">
                        ...
                        <div class="calendar-timeline-wrapper mt-4 week-calender-wrapper">
                        <ul class="calendar-timeline">
                        <li class="timeline-block week-bar">
                                <div class="time-block" style="opacity: 0;">Days</div>
                                <div class="reminder-details-block">Mon</div>
                                <div class="reminder-details-block">Tue</div>
                                <div class="reminder-details-block">Wed</div>
                                <div class="reminder-details-block">Thu</div>
                                <div class="reminder-details-block">Fri</div>
                                <div class="reminder-details-block">Sat</div>
                                <div class="reminder-details-block">Sun</div>
                            </li>
                            @for($i=0; $i<24;$i++)
                            <li class="timeline-block previous mb-3" id="t-{{$i}}">
                                ...
                                @for($j=1;$j<=7;$j++)
                                <div class="reminder-details-block {{jddayofweek($j-1,2)}} {{jddayofweek($j-1,2).'-'.str_pad($i,2,'0',STR_PAD_LEFT)}}">
                                    @if(array_key_exists(str_pad($i,2,'0',STR_PAD_LEFT),$hourlytaskevents))    

                                        @if(array_key_exists($j,$hourlytaskevents[str_pad($i,2,'0',STR_PAD_LEFT)]))  
                                            @foreach($hourlytaskevents[str_pad($i,2,'0',STR_PAD_LEFT)][$j] as $taskevent)
                                            <a href=""  data-bs-toggle="modal" data-bs-target="#taskeventpopup" data-type="{{$taskevent->type}}" data-id="{{$taskevent->id}}" class="reminder-cards-wrapper">
                                                <div class="reminder-card d-flex align-items-start" style="border: 1px solid #8861DC;">
                                                    <div class="image-wrapper me-2">
                                                        <img src="{{asset('app-assets/img/Ellipse 6.png')}}" alt="">
                                                    </div>
                                                    <div class="d-flex flex-column reminder-details">
                                                        <span class="color-bar" style="background-color: #8861DC;"></span>
                                                        <h6 class="reminder-title">{{$taskevent->title}}</h6>
                                                        <!-- <h6 class="mb-0 reminder-category "> - </h6> -->
                                                    </div>
                                                    @if(!empty($taskevent->time1)||!empty($taskevent->time2))
                                                        <div class="reminder-timing ms-auto d-flex flex-column justify-content-center align-items-end align-self-center">
                                                        @if(!empty($taskevent->time1))<span class="time">{{date('g:i a',strtotime($taskevent->date.' '.$taskevent->time1))}} </span>@endif
                                                        @if(!empty($taskevent->time2))<span>&nbsp;-&nbsp;</span>
                                                            <span class="time"> {{date('g:i a',strtotime($taskevent->date.' '.$taskevent->time2))}}</span>@endif
                                                        </div>
                                                    @endif
                                                </div>
                                            </a>
