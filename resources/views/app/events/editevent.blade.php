...
@section('content')
<form action="{{route('app.events.update',['appevent'=>$event->id])}}" method="post" class="add-event-form">
...
<div class="add-event-form-wrapper">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" placeholder=""  value="{{old('title',$event->title)}}">
                            @error('title')
                        ...
                        <div class="mb-3 d-flex field-group align-items-center gap-3">
                            <div class="field-row">
                                ...
                                <input type="date" class="form-control @error('title') is-invalid @enderror" id="date" name="date" placeholder="" value="{{old('date',$event->date)}}" >
                                @error('date')
                                    <div class="invalid-feedback">{{$message}}</div>
                                @enderror
                            </div>
                            <div class="field-row repeat-dropdown-wrapper">
                                ...
                                <div class="dropdown">
                                    ...
                                    <input type="hidden" name="repeat" id="repeat" value="{{old('repeat',$event->repeat)}}">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="multi-inputs d-flex w-100 justify-content-between gap-2 align-items-center">
                                <div class="flex-fill">
                                    <input type="time" class="form-control @error('timeStart') is-invalid @enderror" id="time-start" name="timeStart" placeholder="" value="{{old('timeStart',$event->time1)}}">
                                    @error('timeStart')
                                        <div class="invalid-feedback">{{$message}}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="flex-fill">
                                    <input type="time" class="form-control @error('timeEnd') is-invalid @enderror" id="time-end" name="timeEnd" placeholder="" value="{{old('timeEnd',$event->time2)}}">
                                    @error('timeEnd')
                                        <div class="invalid-feedback">{{$message}}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="mb-3 location-field">
                            ...
                            <input type="text" class="form-control @error('location') is-invalid @enderror" id="location" name="location" placeholder="" value="{{old('location',$event->location)}}">
                            <input type="hidden" name="place_id" id="place_id" value="{{old('location',$event->place_id)}}">
                            <input type="hidden" name="lat" id="lat" value="{{old('location',$event->lat)}}">
                            <input type="hidden" name="long" id="long" value="{{old('location',$event->long)}}">
                            @error('location')
                                    <div class="invalid-feedback">{{$message}}</div>
                                @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" id="description" name="description" cols="30" rows="10" class="w-100">{{old('description',$event->description)}}</textarea>
                        </div>
                                ...
                                @foreach(auth()->user()->familymembers as $fam)
                                    <li class="profile-wrapper @if(in_array($fam->id,$event->members->pluck('id')->toArray())) added @else not-added @endif">
                                        <a href="" class="family-member" data-fammember="#fam-{{$fam->id}}">
                                            ...
                                            <input type="checkbox" name="familymember[]" id="fam-{{$fam->id}}" value="{{$fam->id}}" class="d-none" @if(in_array($fam->id,$event->members->pluck('id')->toArray())) checked="checked" @endif />
                                        </a>
                                    </li>
                                @endforeach
                        ...
                        <div class="form-footer text-left pt-3 mt-3 add-event-footer d-flex align-items-center gap-3">
                            ...
                            <div class="upload-btn ">
                                ...
                                <label for="submit" class="form-label btn btn-dark text-light mb-0">Update Event</label>
